...
function nicFunction() {
    if (reasearcherValidation || nic.value !== "") {
        ...
        var newnic = /^([0-9]{12})+$/;
        if ((!nic.value.match(regexnic) && !nic.value.match(newnic)) || nic.value === "") {
...
function stepTwo() {
    if (accountSelect) {
        if (reasearcherValidation) {
            ...
            //residFunction();
            resorgFunction();
            if (stateNic || stateOccupation || stateNumber || stateBday || stateFileToUpload || stateresorg) {
            ...
            if (stateNic || stateOccupation || stateNumber || stateBday || (nic.value === "" && occupation.value === "" && number.value === "" && bday.value === "")) {
                document.getElementById("nextTwo").click();
            } else {
                document.getElementById("stepTwoError").style.color = "#b20000";
                document.getElementById("stepTwoError").innerHTML = 'Please check once again red colored textboxes';
