...
// check bird id is available --> fetch bird details
if (isset($_GET['birdid'])) {
    require '../CommonFiles/dbConn.php';
    $sql = "SELECT * FROM birddetails WHERE birdid=" . $_GET['birdid'];
    $result = mysqli_query($conn, $sql);
    if ($data = mysqli_fetch_assoc($result)) {
        $id = $data['birdid'];
        $com_name = $data['commonname'];
        $sci_name = $data['scientificname'];
        $sin_name = $data['sinhalaname'];
        $other_name = $data['othername'];
        $details = $data['details'];
        $size = $data['size'];
        $shape_id = $data['shapeid'];
        $redlist_id = $data['redlistid'];

        $sqlShape = "SELECT * FROM shape WHERE shapeid=" . $shape_id;
        $resultShape = mysqli_query($conn, $sqlShape);
        if ($dataShape = mysqli_fetch_assoc($resultShape)) {
            $shape_name = $dataShape['shapename'];
            $shape_img = $dataShape['shapeimage'];
        }

        $sqlRed = "SELECT * FROM redlist WHERE redlistid='" . $redlist_id . "'";
        $resultRed = mysqli_query($conn, $sqlRed);
        if ($dataRed = mysqli_fetch_assoc($resultRed)) {
            $red_cat = $dataRed['category'];
            $red_details = $dataRed['details'];
            $red_id = $dataRed['redlistid'];
        }
    }

    // check user login, type --> and define redlistPass and loginPass
    session_start();
    $redlistPass = FALSE;
    $loginPass = FALSE;
    if (isset($_SESSION["usertype"])) {
        ...
        if ($user_type === 'normal' && $red_id !== 'LC' && $red_id !== 'NT' && $red_id !== 'DD') {
            $redlistPass = FALSE;
        } else {
            $redlistPass = TRUE;
        }
        ...
        $loginPass = FALSE;
        if ($red_id !== 'LC' && $red_id !== 'NT' && $red_id !== 'DD') {
            $redlistPass = FALSE;
        } else {
            $redlistPass = TRUE;
        }
    ...
    // Location Loading -->  and make 3 strings  
    if ($redlistPass) {
        $sqlLoc = "SELECT * FROM blss WHERE birdid=" . $_GET['birdid'];
        ...
        while ($dataLoc = mysqli_fetch_assoc($resultLoc)) {
            $locationid = $dataLoc['locationid'];
    ...
    //check add checklist --> define checklist button
    $checkListbutton = 'disabled';
    if ($loginPass) {
        $checkListSql = "SELECT * FROM checklist WHERE userid=".$user_id." AND birdid=".$id;
        $checkListResult = mysqli_query($conn, $checkListSql);
        if (mysqli_fetch_assoc($checkListResult)) {
            $checkListbutton = "<input id='checkButton' type='button' value='Remove from checklist' onclick='removeCheckList(\"$user_id\",\"$id\")'/>";
        } else {
            $checkListbutton = "<input id='checkButton' type='button' value='Add to checklist' onclick='addCheckList(\"$user_id\",\"$id\")'/>";
        }
    } else {
        $checkListbutton = "<input id='checkButton' type='button' value='Add to checklist' disabled='disabled' />";
    }
    ?>

    <div class="details">
        <img src="../img/birds/<?php echo $id; ?>-1.png">
            <h2><?php echo $com_name; ?></h2>
            <h2><?php echo $sin_name; ?></h2>
            <h3><?php echo $sci_name; ?></h3>
            <h3><?php echo $other_name; ?></h3>
            <?php echo $checkListbutton; ?>
    </div>

    <div class="details">
        <h3><?php echo $details; ?></h3>
        <h3><?php echo $size; ?></h3>
        <h3><?php echo $shape_name; ?></h3>
        <h3><?php echo $red_cat; ?></h3>
        <img src="../img/birds/redList/<?php echo $red_id; ?>.png">
    </div>

    <div class="details" id="hhdmap">
        <?php if ($redlistPass) { ?>
            <div style="width: 80%">

                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20.8 36">
                    <style>
                        .map { stroke: #adadad; stroke-width: 0.05; } 
                        .region { fill: #fff; } 
                        .borders { fill: none;}
                    </style>
                    ...
                    <g class="map">
                        <g class="regions">
                            ...
                            <path class="region" id="dis-13" d="M8.55 21.52c0 .15-.25.25-.33.35-.08.1-.23-.06-.32-.08-.2-.06-.1.57-.54.4-.15-.32-.6-.53-.64-.05-.05.4-.5.52-.86.64-.2.07-.28.4-.5.25-.12-.08-.15-.18-.28-.26-.13-.07-.05-.26-.22-.28-.2 0-.36.16-.5.26-.27.18-.3 0-.56-.03-.18-.03-.32.15-.5.15-.26-.32-.1-.9-.22-1.28-.15-.42-.23-.92-.15-1.36.03-.2.2-.88.4-.77.15.07.05-.2 0-.25-.16-.13.1-.2.2-.27.2-.17.07-.66.2-.75.24-.15.55-.66.7-.9l.3-.28c.1-.07.33.1.35-.08.02-.18-.16-.17 0-.36.1-.14-.02-.43 0-.6 0-.15-.2-.2-.28-.3-.12-.15-.13-.66-.14-.86.36.35 1 .4 1.46.6.35.16.6.55.97.63.43.1.25.42.33.77.03.13.2.08.3.16.13.14.18.4.2.6 0 .14-.07.3-.03.44.02.08.07.13.16.14-.04.1-.04.17 0 .26.16.3.36.56.5.88.17.45-.18 1.02.04 1.4.22.4-.05.5-.05.8 0 .12 0 0 0 0z"/>
                            <g class="region" id="dis-14">
                            ...
                            <path class="region" id="dis-22" d="M10.66 27.3c.63-.07.4.15.2.53-.13.23.04.46.3.46.12-.02.37-.35.47-.3 0 0-.43 1.05-.46 1.16-.1.28-.34.45-.3.77.06.33-.04.75.26.97.2.14 1 1.22.38 1.06-.12-.04-.23-.04-.3-.17-.1-.2-.4-.08-.57-.1-.2-.03-.4-.05-.56-.14-.2-.1-.4-.1-.6-.17.14-.16.08-.58-.17-.47-.05.02-.38.1-.2-.04.1-.1-.13-.1-.1-.2 0-.1.15-.18-.08-.16-.24.03-.5.03-.67.2-.1.07-.83-.13-1-.24-.3-.2-.33-.47-.54-.8-.13-.2-.46-.48-.3-.6.15-.12-.18-.37-.24-.4-.24-.13-.17-.35-.34-.53-.26-.3-.45-.75-.5-1.14.22.03.15-.2.1-.3-.06-.2.13-.5 0-.6-.1-.13 0-.38.17-.34.16.02.33.46.45.6.13.12.25.46.37.54.12.08.38-.16.47-.22.42-.28.75.34 1.12.13.04.18 0 .47.26.47.25 0 .87.2 1.07.12.24-.1.44 0 .68-.05.22-.06.43-.1.63-.1.1-.02-.04 0 0 0z"/>
                            <g class="region" id="dis-23">
                ...
                <div style="display: none">
                    <input type="text" name="st1" id="st1" value="<?php echo $st1; ?>" />
                    <input type="text" name="st2" id="st2" value="<?php echo $st2; ?>" />
                    <input type="text" name="st3" id="st3" value="<?php echo $st3; ?>" />
                </div>
            </div>
        <?php } else { ?>
            You are not allowed to show this map
        <?php } ?>
    </div>
    <?php
} else {
    echo '<img src="../img/index/error.png">';
}
